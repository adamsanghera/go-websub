...
func (sc *Server) CallbackSwitch(w http.ResponseWriter, req *http.Request) {
...
func (sc *Server) handleSubscription(w http.ResponseWriter, endpoint string, query url.Values) {
...
func (sc *Server) handleDeniedSubscription(query url.Values) {
	topic := query.Get("hub.topic")
	...
	if _, isPending := sc.pendingSubs[topic]; isPending {
		...
		if sub, isActive := sc.activeSubs[topic]; isActive {
			...
			// This might happen if a renewal thread is attempting to renew, but is denied.
			// In this case, cancel won't do anything.
			sub.cancel()
...
func (sc *Server) handleUnsubscription(w http.ResponseWriter, endpoint string, query url.Values) {
	topic := query.Get("hub.topic")
	challenge := query.Get("hub.challenge")
...
func (sc *Server) renewSubscription(ctx context.Context, topic string) {
